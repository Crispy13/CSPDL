...
class DecoderBlock_GC(keras.layers.Layer):
    def __init__(self, cardinality, filters, data_format = 'channels_last', kernel_initializer = "he_normal", name = "", first_conv2d_strides = (1, 1), **kwargs):
        ...
        self.conv_reduce_bn = keras.layers.BatchNormalization(axis = -1, name = f"{self.name}_conv_reduce_bn")
        self.conv_reduce_relu = keras.layers.ReLU(name = f"{self.name}_conv_reduce_relu")
        ...
        self.conv_group = keras.layers.Conv2D(filters = filters * 2, kernel_size = (3, 3), strides=(1, 1), padding = 'valid', 
                                            data_format = data_format, kernel_initializer = kernel_initializer, name = f"{self.name}_group_conv",
                                             groups = cardinality)
        self.conv_group_bn = keras.layers.BatchNormalization(axis = -1, name = f"{self.name}_conv_group_bn")
        self.conv_group_relu = keras.layers.ReLU(name = f"{self.name}_conv_group_relu")
    ...
    def call(self, inputs):
        ...
        Z = self.conv_reduce_bn(Z)
        Z = self.conv_reduce_relu(Z)
        
        Z = self.conv_group(Z)
        Z = self.conv_group_bn(Z)
        Z = self.conv_group_relu(Z)
...
### Preact Resnet50 Segmentation model in https://arxiv.org/abs/1810.13230
class PreactResnetModified5(keras.Model):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
...
class DiceLoss(keras.losses.Loss):
    ...
    def call(self, y_true, y_pred):
        ...
        return 1 - tf.reduce_mean((2 * intersection + self.epsilon) / (dn + self.epsilon), axis = -1)
...
class LogLogger(keras.callbacks.Callback):
    ...
    def on_train_batch_end(self, batch, logs=None):
        self.logger.info(f"Batch {batch}: " + ",   ".join([f"{k} = {v:7.4f}" for k,v in logs.items()]))
#         self.logger.info("For batch {}: loss = {:7.4f}, metric = {:7.4f}.".format(batch, logs[self.loss_name], logs[self.metric_name]))
...
###
def IoULoss_(calc_axis = (-3, -2, -1), smooth = 1e-6):
...
class DiceCoefficient(keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        tf.print(y_pred.shape, y_true.shape)
        
        intersection = tf.reduce_sum(tf.abs(y_true * y_pred), axis = self.calc_axis) + self.epsilon
        dn = tf.reduce_sum(tf.square(y_true) + tf.square(y_pred), axis = self.calc_axis) + self.epsilon
        
        dice = tf.reduce_mean(2 * intersection / dn, axis = None)
